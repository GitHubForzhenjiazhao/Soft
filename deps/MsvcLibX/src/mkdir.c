...
int mkdirA(const char *pszName, mode_t iMode) {
  ...
  //[3/21/2022 15:21:06 zhenjia.zhao modified]
  WCHAR wszNametmp[PATH_MAX];
  memset(wszNametmp, 0, PATH_MAX);
  for (int strCount = 0; strCount < wcslen(wszName); strCount++) 
  {
    if (wszName[strCount] == '\\') 
	{
      bDone = CreateDirectoryW(wszNametmp, NULL);
      if (!bDone) 
	  {
        DEBUG_PRINTF("CreateDirectoryW(\"%s\") failed;", pszName);
      }
    } 
	wszNametmp[strCount] = wszName[strCount]; 
  }
  bDone = CreateDirectoryW(wszName, NULL);
  if (!bDone) 
  {
  ...
  //end
  //[3/21/2022 13:54:24 zhenjia.zhao delete]
  //bDone = CreateDirectoryW(wszName, NULL);
  //[3/23/2022 9:01:46 zhenjia.zhao modified]
  //if (!bDone) {
  //  errno = Win32ErrorToErrno();
  //  return -1;
  //}
