...
static void state_count_function(SQLFunctionCtx *pCtx) {
  ...
  char *data = GET_INPUT_DATA_LIST(pCtx);//[3/21/2022 11:06:43 zhenjia.zhao modified]void* changed to be char*
  int64_t *pOutput = (int64_t *)pCtx->pOutput;
...
static void state_duration_function(SQLFunctionCtx *pCtx) {
  ...
  char *data = GET_INPUT_DATA_LIST(pCtx);//[3/21/2022 11:07:27 zhenjia.zhao modified]void* changed to be char*
  TSKEY* tsList = GET_TS_LIST(pCtx);
  int64_t *pOutput = (int64_t *)pCtx->pOutput;
